...
- **Pre-connect indicative rate** — see an oracle-based estimated rate before connecting your wallet, so you know what to expect before signing in
- **Fee disclosure on quote step** — the fee is shown as `0.05% · $X.XX` before you confirm, with no surprises at settlement
- **TradFi terminology** — the interface uses familiar foreign exchange language: "Get FX Rate", "Confirm Exchange", "Rate tolerance", and "Exchange settings" instead of DeFi jargon
- **Gasless relay** — swap without holding ETH for gas; Permit2 + relay router handle submission on your behalf
